...
A feedback launcher + dialog. The launcher renders as a vertical tab flush against the right viewport edge by default, or as the classic floating corner bubble via `variant="bubble"`. The dialog has a textarea, a screenshot capture button with a region-select overlay (drag a rectangle, or "Capture full page"), file attachments, and a send button. Captured screenshots render as thumbnail chips with a tap-to-preview dialog.
