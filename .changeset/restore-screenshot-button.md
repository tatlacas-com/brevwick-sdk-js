...
The screenshot capture button — removed in v1 behind a future-flag (PR #111) — is back across all five web widgets (React, Solid, Vue, Svelte, Angular), backed by the core SDK's lazy `captureScreenshot()` wrapper. Clicking the camera button captures the page via the dynamically imported `modern-screenshot` peer dep, shows a preview dialog with an optional region-select overlay, and attaches the resulting image to the submitted issue.
